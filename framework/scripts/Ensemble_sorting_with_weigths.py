...
#  scoring function
metrics_dict = {'NSE': {'BestValue': 1.0, 'Weight': 10.0},
...
for column in df.columns:
    df_eval['{}_eval'.format(column)] = abs(df_eval[column]-metrics_dict['{}'.format(column)]['BestValue'])
    df_eval['{}_Score'.format(column)] = df_eval['{}_eval'.format(column)].rank(ascending=False)

df_eval['Score'] = 0
weights_sum = 0
for column in df.columns:
    df_eval['Score'] += df_eval['{}_Score'.format(column)]*metrics_dict['{}'.format(column)]['Weight']
