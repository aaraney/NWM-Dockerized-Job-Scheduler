...
class Scheduler(object):
    ...
    def from_list(cls, primary_dir, alt_domain_list):
        ...
        for i, fn in enumerate(alt_domain_list):
            ...
            replica_mnt_point = join(
                dirname(primary_dir), "rep-{}-sesh-{}".format(i, scheduler.schedule_id)
            )
    ...
    def from_yaml(cls, yaml_file):
        ...
        try:
            ...
            raise KeyError(
                "Check that both `primary` and `alternative-files` are keys in yaml file."
            )

        # Set class property if provided in yaml
        optional_keys = {
            "cpus": "scheduler.max_cpus",
            "image": "scheduler.image_tag",
            "max-jobs": "scheduler.max_jobs",
            "mpi-np": "scheduler.mpi_np",
        }
    ...
    def _check_heart_beat(self):
        ...
        try:
            ...
            raise ConnectionError(
                "Please check that the Docker Daemon is installed and running."
            )
    ...
    def check_for_image(self, image_tag):
        ...
        docker_repo_tag_list = reduce(
            operator.concat, list(map(lambda x: x.attrs["RepoTags"], docker_image_list))
        )
    ...
    def _run_job(self, job, image_tag, cpuset):
        volumes = {job.replica_mnt_point: {"bind": "/replica", "mode": "rw"}}
        job.container_id = self.docker_client.containers.run(
            image_tag,
            cpuset_cpus=cpuset,
            detach=True,
            entrypoint="run.sh {}".format(self.mpi_np),
            remove=True,
            volumes=volumes,
        )
        return job
    ...
    def start_jobs(self):
        ...
        if len(self.docker_client.containers.list()) > self._MAX_JOBS:
            raise Exception(
                "System already has too many running containers. "
