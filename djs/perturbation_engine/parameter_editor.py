...
def _check_parameter_validity(parameter_file: Union[str, xr.core.dataset.Dataset]):
    ...
    # KeyError thrown by filehandler if not a valid NWM file
        ...
        raise IOError(
            "The provided parameter file, {}, is not valid, please provide a valid WRF-Hydro/NWM parameter file".format(
                basename(parameter_file._file_obj._filename)
            )
        )
...
def _map_to_operator(op: str):
    ...
    try:
        ...
        raise (
            KeyError(
                'Operator or ditribution "{}" not supported. Please use standard python numeric operators'
            )
        )
    ...
    for parameter, operator_value_list in parameter_operator_dict.items():
        ...
        for func_value_pair in operator_value_list:
            ...
            if "perterbation_engine_edits:" in local_df.attrs:
                local_df.attrs["perterbation_engine_edits:"] += _metadata_string(
                    parameter, str_func, value
                )
...
def _apply_dists(
    df: xr.core.dataset.Dataset,
    parameter_operator_dict: Dict[str, Tuple[Union[str, bool]]],
    ...
    if apply_bool:
        rvs = eval(
            f"{dist_type}.rvs(*mle_fit, size = parameter_size).reshape(*parameter_shape)"
        )
    else:
...
def perturb_parameters(
    df: Union[str, xr.core.dataset.Dataset],
    parameter_operator_dict: Dict[str, List[Tuple[Union[bool, str, float, int]]]],
) -> xr.core.dataset.Dataset:
    """
    ...
    if not len(parameter_intersection):
        raise IOError(
            "Input parameters {} are not valid".format(
                (set(parameter_operator_dict.keys()) - valid_parameters)
            )
        )
